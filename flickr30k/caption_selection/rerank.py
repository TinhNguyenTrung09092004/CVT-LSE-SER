...
EVIDENCE GUIDE:
- Objects (YOLO, conf >= 0.6 — HIGHLY RELIABLE): Exact object categories (COCO-format labels) with counts.
...
def load_detection_data(split: str = "test"):
    ...
    hoi_path   = os.path.join(DETECTION_DATASET_DIR, 'hoi',   f'flickr30k_{split}_hoi_detection.csv')
    reltr_path = os.path.join(DETECTION_DATASET_DIR, 'reltr', f'flickr30k_{split}_reltr_detection.csv')
    yolo_path  = os.path.join(DETECTION_DATASET_DIR, 'yolo',  f'flickr30k_{split}_detection.csv')
...
CSV_PATH = "outputs/flickr30k/beam_candidates_epoch_<EPOCH>.csv"
...
# %%
RERANK_CSV_PATH = 'outputs/flickr30k/flickr30k_llm_rerank_selections.csv'
