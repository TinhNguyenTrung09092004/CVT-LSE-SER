...
# === SPLIT ===
# 0.1  -> generate descs for test
# 0.2  -> generate descs for val
# 1    -> train part 1/10
# 2    -> train part 2/10
# ...
# 10   -> train part 10/10
...
def _parse_split(s):
    """Return (main_part, sub_part): 0.1=test, 0.2=val, 1-10=train."""
    if s < 1:
        return 0, round(s * 10)
    return int(s), 0

_main, _sub = _parse_split(SPLIT)
assert (_main == 0 and _sub in [1, 2]) or (1 <= _main <= 10 and _sub == 0), \
    "SPLIT must be 0.1 (test), 0.2 (val), or 1-10 (train)"

if _main == 0:
    _split_label = "split0_test" if _sub == 1 else "split0_val"
else:
    _split_label = f"split{_main}_train"
os.makedirs(DESC_DIR, exist_ok=True)
INTERMEDIATE_CSV = os.path.join(DESC_DIR, f"flickr30k_descs_{_split_label}.csv")
...
def load_detection_for_split(split: str) -> Dict[str, Dict]:
    yolo_path  = os.path.join(DETECTION_DIR, "yolo",  f"flickr30k_{split}_detection.csv")
    hoi_path   = os.path.join(DETECTION_DIR, "hoi",   f"flickr30k_{split}_hoi_detection.csv")
    reltr_path = os.path.join(DETECTION_DIR, "reltr", f"flickr30k_{split}_reltr_detection.csv")
...
if _main == 0:
    if _sub == 1:
        all_images = test_imgs
        print(f"Split 0.1 (test): {len(all_images):,} images")
    else:
        all_images = val_imgs
        print(f"Split 0.2 (val): {len(all_images):,} images")
else:
    total     = len(train_imgs_sorted)
    part_size = (total + 9) // 10
    start     = (_main - 1) * part_size
    end       = min(_main * part_size, total)
    all_images = train_imgs_sorted[start:end]
    print(f"Split {_main} (train part {_main}/10): images [{start:,}-{end:,}), total {len(all_images):,} images")

if PREVIEW_MODE:
    all_images = all_images[:PREVIEW_N]
print(f"Total images to generate descs for: {len(all_images):,}")
...
intermediate_rows = []
for batch_start in tqdm(range(0, len(all_images), QWEN_BATCH_SIZE), desc="Generating descs (Qwen)"):
...
print(f"Saved intermediate descs: {len(intermediate_df):,} images -> {INTERMEDIATE_CSV}")
