...
def get_image_path(img_info: dict) -> str:
    return os.path.join(IMAGE_DIR, img_info["filename"])
...
def load_split_paths() -> Dict[str, List[str]]:
    ...
    for img_info in data["images"]:
        ...
        if split in splits and path not in seen[split]:
...
def parse_part(part: str) -> Tuple[str, Optional[int]]:
    if part.startswith("1."):
        ...
        assert 0 <= idx <= 1, f"Train part must be 1.1~1.2, got: {part}"
        return "train", idx
    elif part == "2":
        return "test", None
    elif part == "3":
...
if chunk_idx is not None:
    image_paths = chunk_list(image_paths, 2)[chunk_idx]
    label = f"train part {chunk_idx + 1}/2"
else:
    label = split
