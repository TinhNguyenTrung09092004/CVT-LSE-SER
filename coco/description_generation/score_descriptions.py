...
# === TARGET ===
# "val"     → evaluate coco_descs_val_merged.csv
# "test"    → evaluate coco_descs_test_merged.csv
# "train_A" → evaluate coco_descs_train_merged_A.csv
# "train_B" → evaluate coco_descs_train_merged_B.csv
# "train_C" → evaluate coco_descs_train_merged_C.csv
TARGET = "train_A"
assert TARGET in ("val", "test", "train_A", "train_B", "train_C"), \
    "TARGET must be 'val', 'test', 'train_A', 'train_B', or 'train_C'"

_input_map = {
    "val":     "coco_descs_val_merged.csv",
    "test":    "coco_descs_test_merged.csv",
    "train_A": "coco_descs_train_merged_A.csv",
    "train_B": "coco_descs_train_merged_B.csv",
    "train_C": "coco_descs_train_merged_C.csv",
}
INTERMEDIATE_CSV = os.path.join(DESC_DIR, _input_map[TARGET])
OUTPUT_CSV       = os.path.join(DESC_DIR, f"coco_descs_{TARGET}_scored.csv")
...
def score_descs_blip(image_path: str, descs: List[str]) -> List[float]:
    """Return the ITM score (match probability) of each description with the image."""
    try:
...
final_rows = []
for _, row in tqdm(intermediate_df.iterrows(), total=len(intermediate_df), desc="Evaluating BLIP ITM"):
    img      = row["image"]
