...
# %%
df_val = detect_and_save_csv(val_images, "VAL", os.path.join(OUTPUT_DIR, "coco_val_hoi_detection.csv"))

display_sample_with_predictions(val_images, df_val, "VAL", n=20)

print("Preview coco_val_hoi_detection.csv (first 20 rows):")
print(df_val.head())

# %%
df_test = detect_and_save_csv(test_images, "TEST", os.path.join(OUTPUT_DIR, "coco_test_hoi_detection.csv"))

display_sample_with_predictions(test_images, df_test, "TEST", n=20)

print("Preview coco_test_hoi_detection.csv (first 20 rows):")
print(df_test.head())

# %%
df_train = detect_and_save_csv(train_images, "TRAIN", os.path.join(OUTPUT_DIR, "coco_train_hoi_detection.csv"))

display_sample_with_predictions(train_images, df_train, "TRAIN", n=20)

print("Preview coco_train_hoi_detection.csv (first 20 rows):")
print(df_train.head())

# %%
print(f"TRAIN: {len(train_images):,} images | {len(df_train):,} predictions | {df_train['prediction'].nunique()} HOI classes")
print(f" TEST:  {len(test_images):,} images | {len(df_test):,} predictions | {df_test['prediction'].nunique()} HOI classes")
print(f" VAL:   {len(val_images):,} images | {len(df_val):,} predictions | {df_val['prediction'].nunique()} HOI classes")
